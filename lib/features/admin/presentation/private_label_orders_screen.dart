...
class PrivateLabelOrdersScreen extends StatefulWidget {
  const PrivateLabelOrdersScreen({super.key});

  @override
  State<PrivateLabelOrdersScreen> createState() => _PrivateLabelOrdersScreenState();
}

class _PrivateLabelOrdersScreenState extends State<PrivateLabelOrdersScreen> {
  ...
  void initState() {
    super.initState();
    RouteTracker.saveRoute('private_label_orders');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Private Label Orders'),
        elevation: 0,
      ...
      body: BlocBuilder<OrderBloc, OrderState>(
        builder: (context, state) {
          if (state is OrderLoading) {
            ...
            // FILTER FOR PRIVATE LABEL ONLY
            sortedOrders = sortedOrders.where((o) => o.orderType == 'private_label').toList();
              ...
              return InkWell(
                onTap: () {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => OrderDetailsScreen(order: order)))
                      .then((_) => RouteTracker.saveRoute('private_label_orders'));
                },
                child: Card(
                  elevation: 2,
